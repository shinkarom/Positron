...
if(BUILD_SDL)
    ...
    if(WIN32)
        ...
        add_executable(positron ${SYSTEM_TYPE} ${TIC80_SRC})
    ...
    elseif(ANDROID)
        ...
        add_executable(positron ${TIC80_SRC})
    endif()

    if(MINGW)
        target_link_libraries(positron mingw32)
        target_link_options(positron PRIVATE -static -mconsole)
    endif()

    if(EMSCRIPTEN)
        set_target_properties(positron PROPERTIES LINK_FLAGS "-s WASM=1 -s USE_SDL=2 -s ALLOW_MEMORY_GROWTH=1 -s FETCH=1 --pre-js ${CMAKE_SOURCE_DIR}/build/html/prejs.js -lidbfs.js")
    ...
    elseif(NOT ANDROID)
        target_link_libraries(positron SDL2main)
    endif()

    target_link_libraries(positron tic80studio)

    if(BUILD_TOUCH_INPUT)
        target_compile_definitions(positron PRIVATE TOUCH_INPUT_SUPPORT)
    endif()

    if (FREEBSD)
        target_include_directories(positron PRIVATE ${SYSROOT_PATH}/usr/local/include)
        target_link_directories(positron PRIVATE ${SYSROOT_PATH}/usr/local/lib)
    endif()

    if(RPI)
        target_include_directories(positron PRIVATE ${SYSROOT_PATH}/usr/local/include/SDL2)
        target_link_directories(positron PRIVATE ${SYSROOT_PATH}/usr/local/lib ${SYSROOT_PATH}/opt/vc/lib)
        target_compile_definitions(positron PRIVATE __RPI__)
    endif()

    if(BUILD_SDLGPU)
        target_link_libraries(positron sdlgpu)
    else()
        if(EMSCRIPTEN)
        elseif(RPI)
            target_link_libraries(positron libSDL2.a bcm_host)
        else()
            target_link_libraries(positron SDL2-static)
        endif()
    endif()
